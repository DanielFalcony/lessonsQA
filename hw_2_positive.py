from hw_2_checkers import (checkout, take_data, folder_in,
                           folder_out, folder_ext, folder_ext2, make_folders, make_files,
                           make_subfolder, clear_folders)
...
def test_step2(clear_folders, make_files):
    ...
    for item in make_files:
        ...
        hash_date_crc32 = str(take_data(f"cd {folder_in}; crc32 {item}")).upper()
        res.append(checkout(f"cd {folder_in}; 7z h {item}", hash_date_crc32))
...
def test_step5(clear_folders, make_files, make_subfolder):
    ...
    res.append(checkout(f"ls {folder_ext2}", make_subfolder[0]))
    res.append(checkout(f"ls {folder_ext2}", make_subfolder[1]))
    assert all(res), "test5 FAIL"


def test_step6():
    # Test integrity of archive
    assert checkout(f"cd {folder_out}; 7z t arx.7z", "Everything is Ok"), "test3 FAIL"


def test_step7():
    # test Update files to archive
    assert checkout(f"cd {folder_in}; 7z u arx.7z", "Everything is Ok"), "test4 FAIL"


def test_step8():
    # test Delete files from archive
    assert checkout(f"cd {folder_out}; 7z d arx.7z", "Everything is Ok"), "test5 FAIL"
